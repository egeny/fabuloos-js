...
/*global browser, rhino, window, navigator, ActiveXObject */
...
*/
Renderer.init = function( config, instance ) {

	// TODO

	var
		klass = instance.constructor,
		instances = Renderer.instances[klass.name];
...
Renderer.canPlay = function( url ) {
	var
		rExt = url.match( /\.([\w\d]+)(?=\/|\?|$)/g ),
		ext  = rExt[rExt.length - 1].substring( 1 ),
	...
	*/
	set: function( property, value ) {
		...
		if (this.isReady) {
			...
			// Initialize the propertyCache if doesn't exists
			if (!this.propertyCache) {
				this.propertyCache = {};
			}

			// If the element doesn't exists (not ready or in the DOM), store properties and values in a cache
